"""
This recipe supports two corpora: wenet and wenet-R.

---

wenet is a multi-speaker English corpus of approximately 585 hours of read English speech at 24kHz sampling rate, prepared by Heiga Zen with the assistance of Google Speech and Google Brain team members. The wenet corpus is designed for TTS research. It is derived from the original materials (mp3 audio files from LibriVox and text files from Project Gutenberg) of the LibriSpeech corpus. The main differences from the LibriSpeech corpus are listed below:
...
For more information, refer to the paper "wenet: A Corpus Derived from LibriSpeech for Text-to-Speech", Heiga Zen, Viet Dang, Rob Clark, Yu Zhang, Ron J. Weiss, Ye Jia, Zhifeng Chen, and Yonghui Wu, arXiv, 2019. If you use the wenet corpus in your work, please cite this paper where it was introduced.

---

wenet-R [1] is a sound quality improved version of the wenet corpus (http://www.openslr.org/60/) which is a multi-speaker English corpus of approximately 585 hours of read English speech at 24kHz sampling rate, published in 2019. The constituent samples of wenet-R are identical to those of wenet, with only the sound quality improved. To improve sound quality, a speech restoration model, Miipher proposed by Yuma Koizumi [2], was used.

For more information, refer to the paper [1]. If you use the wenet-R corpus in your work, please cite the dataset paper [1] where it was introduced.

Audio samples of the ground-truth and TTS generated samples are available at the demo page: https://google.github.io/df-conformer/wenetr/

[1] Yuma Koizumi, Heiga Zen, Shigeki Karita, Yifan Ding, Kohei Yatabe, Nobuyuki Morioka, Michiel Bacchiani, Yu Zhang, Wei Han, and Ankur Bapna, "wenet-R: A Restored Multi-Speaker Text-to-Speech Corpus," arXiv, 2023.
...
def download_wenetr(
    ...
    """
    Download and untar the wenet-R dataset.
...
def download_wenet(
    target_dir: Pathlike = ".",
    use_wenetr: bool = False,
    dataset_parts: Optional[Union[str, Sequence[str]]] = "all",
    force_download: Optional[bool] = False,
    base_url: Optional[str] = "http://www.openslr.org/resources",
) -> Path:
    """
    Download and untar the wenet dataset.

    :param target_dir: Pathlike, the path of the dir to storage the dataset.
    :param use_wenetr: Bool, if True, we'll download the wenet-R dataset instead.
    :param dataset_parts: "all", or a list of splits (e.g. "dev-clean") to download.
    :param force_download: Bool, if True, download the tars no matter if the tars exist.
    :param base_url: str, the url of the OpenSLR resources.
    :return: the path to downloaded and extracted directory with data.
    ...
    for part in tqdm(dataset_parts, desc=f"Downloading {name} parts"):
        if part not in wenet:
            logging.warning(
                f"Skipping invalid dataset part name: {part} (possible choices: {wenet})"
            )
...
def prepare_wenet(
    ...
    :param link_previous_utt: If true adds previous utterance id to supervisions.
        ...
        If previous utterance was skipped from wenet datasets previous_utt label is None.
    ...
    if output_dir is not None:
        ...
        manifests = read_manifests_if_cached(
            dataset_parts=dataset_parts, output_dir=output_dir, prefix="wenet"
        )
    ...
    for part in tqdm(dataset_parts, desc="Preparing wenet parts"):
        ...
        # Maybe wenet-R will fix it in later distributions.
        ...
        for trans_path in tqdm(
            ...
            #     continue
            # The trans.tsv files contain only the recordings that were kept for wenet.
            # Example path to a file:
            #   /export/corpora5/wenet/dev-clean/84/121123/84_121123.trans.tsv
            #
            # Example content:
            ...
            for line in trans_path.read_text().splitlines():
                print(line)
                rec_id  = trans_path.name.replace(".txt","")
        ...
        if output_dir is not None:
            supervisions.to_file(output_dir / f"wenet_supervisions_{part}.jsonl.gz")
            recordings.to_file(output_dir / f"wenet_recordings_{part}.jsonl.gz")
